...
def create_df_split(csv_name, label, rnd_state=10):
	...
	#df_det = df[df[label] == 1]
	#df_det_split = np.array_split(df_det, 2)
	#print(df_det_split[0])
	#print(df_det_split[1])
	#df_und = df[df[label] == 0]
	#df_und_split = np.array_split(df_und, 2)
	#print(df_und_split[0])
	#print(df_und_split[1])
...
def drop_labels(df, label):
	...
	if label == "is_succ":
		...
		df['is_succ'] = df['is_succ'].astype('category')
	if label == "magnet_det":
		df = df.drop('is_succ', axis=1)
		df = df.drop('squeezer_det', axis=1)
		df['magnet_det'] = df['magnet_det'].astype('category')
	if label == "squeezer_det":
		df = df.drop('is_succ', axis=1)
		df = df.drop('magnet_det', axis=1)
		df['squeezer_det'] = df['squeezer_det'].astype('category')
...
def run_classifier(x_train, y_train, x_test, y_test, features, id_exp, label, param=0):
	...
	clf = XGBClassifier(n_estimators=90, random_state=0) #, tree_method= 'gpu_hist'
	clf.fit(x_train, y_train)

	attacks_names = x_test.file_name.unique()
	x_test = x_test.reset_index() 

	df_packet = pd.DataFrame(columns=['id', 'label', 'features', 'file_name', 'csv_name','attack', 'dataset_name', 'model_name' 'ACC'])
	df_attack = pd.DataFrame(columns=['id', 'label', 'features', 'file_name', 'csv_name', 'attack', 'dataset_name', 'model_name' 'ACC'])
	df_all = pd.DataFrame(columns=['file_name', 'ACC'])
	"""
	...
	for name in attacks_names:
		...
		acc = accuracy_score(y_test_atk, y_pred)
	...
	y_pred = clf.predict(x_test_atk)
	acc = accuracy_score(y_test_atk, y_pred)
	mcc = matthews_corrcoef(y_test_atk, y_pred)
	...
	new_row = pd.DataFrame({'id': id_exp, 'label': label, 'features': ' '.join(features), 'csv_name': 'all', 
			...
			'model_name': 'conv12', 'ACC': acc, 'MCC': mcc}, index=[1])
