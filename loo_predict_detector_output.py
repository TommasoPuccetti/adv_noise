...
def run_classifier(x_train, y_train, x_test, y_test, features, id_exp, label, param=0):
	...
	clf = XGBClassifier(n_estimators=90, random_state=0, eval_metric='auc') #, tree_method= 'gpu_hist'
	clf.fit(x_train, y_train)

	attacks_names = x_test.file_name.unique()
	x_test = x_test.reset_index() 

	df_packet = pd.DataFrame(columns=['id', 'label', 'features', 'file_name', 'csv_name','attack', 'dataset_name', 'model_name', 'ACC', 'MCC'])
	df_attack = pd.DataFrame(columns=['id', 'label', 'features', 'file_name', 'csv_name', 'attack', 'dataset_name', 'model_name', 'ACC', 'MCC'])
	df_all = pd.DataFrame(columns=['file_name', 'ACC', 'MCC'])
	...
	for name in attacks_names:
		...
		new_row = pd.DataFrame({'id': id_exp, 'label': label, 'features': ' '.join(features), 'csv_name': csv_name, 
			...
			'model_name': model_name, 'ACC': acc, 'MCC': mcc}, index=[1])
	...
	for att in attacks_type:
		...
		new_row = pd.DataFrame({'id': id_exp, 'label': label, 'features': ' '.join(features), 'csv_name': csv_name, 
			'attack': attack, 'file_name': name, 'dataset_name': dataset_name,
			'model_name': model_name, 'ACC': acc, 'MCC': mcc, 'test_shape': x_test_atk.shape[0], 'train_shape': x_train.shape[0]}, index=[1])
